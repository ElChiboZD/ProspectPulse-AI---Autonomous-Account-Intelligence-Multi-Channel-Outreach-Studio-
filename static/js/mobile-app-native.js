 ...
 * Complete mobile implementation of all flagship features: Live Cloud Intel, Multi-Channel Studio,
 * Buying Committee Org Tree, Interactive SVG Deal Room, Voice Arena, and Google OAuth 2.0.
 */
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', initNativeMobileApp);

  function initNativeMobileApp() {
    renderMobileDOM();
    setupGoogleIdentity();
  }
...
${p.title} | ${p.company}</div>
  ...
  function updateStudioContent() {
    ...
    if (ch === 'email') {
      ...
      bodyEl.textContent = `Hi Michael — saw you are leading partner experience at ${acc.name}. We helped similar retail leaders eliminate catalog swag markup with custom Italian-spun knitwear.\n\nCreated a preview deal room for your team: [Link]. Open to connecting?`;
    } else {
      if (subEl) subEl.textContent = `PHONE COLD CALL SCRIPT`;
      bodyEl.textContent = `"Hi Michael, Travis with ${p.company}. Quick question — are you the person overseeing store partner merchandise and gifting programs for ${acc.name}?\n\nReason for my call: Most retail teams are spending $185/box on catalog middlemen with 6-week delays. We spin direct custom knitwear at 40% lower cost. Do you have 2 minutes to hear how we work with peer brands?"`;
    }
  }
  ...
  // Google OAuth 2.0 Identity Setup
  function setupGoogleIdentity() {
    setTimeout(() => {
      try {
        if (window.google && window.google.accounts && window.google.accounts.id) {
          window.google.accounts.id.initialize({
            client_id: localStorage.getItem('prospectpulse_google_client_id') || '407408718192.apps.googleusercontent.com',
            callback: handleGoogleAuthCallback,
            auto_select: false
          });
        }
      } catch (e) {
        console.warn('[Google OAuth] GSI setup:', e);
      }
    }, 200);
  }

  function handleGoogleAuthCallback(res) {
    try {
      const base64Url = res.credential.split('.')[1];
      const base64 = base64Url.replace(/-/g, '+').replace(/_/g, '/');
      const payload = JSON.parse(decodeURIComponent(atob(base64).split('').map(c => '%' + ('00' + c.charCodeAt(0).toString(16)).slice(-2)).join('')));
      
      window.MobileApp.userProfile.name = payload.name;
      window.MobileApp.userProfile.email = payload.email;
      if (payload.picture) {
        window.MobileApp.userProfile.avatar = payload.picture;
        const avatarEl = document.getElementById('topAvatarImg');
