...
${profile.title} | ${profile.company}</div>
  ...
  window.openGoogleAuthModal = function () {
    ...
    const currentClientId = localStorage.getItem('prospectpulse_google_client_id') || '407408718192.apps.googleusercontent.com';
    ...
    `;
    modal.innerHTML = `
      <div style="background:#0F121A;border:1px solid rgba(255,255,255,0.12);border-radius:24px;width:100%;max-width:380px;padding:22px;box-shadow:0 20px 40px rgba(0,0,0,0.8);max-height:90vh;overflow-y:auto;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
          <div style="display:flex;align-items:center;gap:10px;">
            <svg width="24" height="24" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"/></svg>
            <strong style="font-size:17px;color:#FFF;">Google OAuth & Gemini</strong>
          </div>
          <button onclick="document.getElementById('googleAuthModal').remove()" style="background:transparent;border:none;color:#9CA3AF;font-size:20px;cursor:pointer;">✕</button>
        </div>

        <div style="font-size:12px;color:#9CA3AF;margin-bottom:14px;">Sign in securely with Google OAuth 2.0 to personalize outreach signatures and link your Google Gemini AI Cloud key.</div>

        <!-- 1. Official Google OAuth 2.0 Button -->
        <div style="margin-bottom:16px;display:flex;flex-direction:column;align-items:center;">
          <div id="gsiButtonWrapper" style="width:100%;min-height:44px;display:flex;justify-content:center;"></div>
        </div>

        <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;">
          <div style="flex:1;height:1px;background:rgba(255,255,255,0.1);"></div>
          <span style="font-size:11px;color:#6B7280;text-transform:uppercase;font-weight:700;">Or Direct Profile Setup</span>
          <div style="flex:1;height:1px;background:rgba(255,255,255,0.1);"></div>
        </div>
    ...
    // Initialize Official Google Identity Services OAuth
    setTimeout(() => {
      try {
        if (window.google && window.google.accounts && window.google.accounts.id) {
          ...
          const btnEl = document.getElementById('gsiButtonWrapper');
          if (btnEl) {
        ...
        console.warn('[Google OAuth] GSI initialization fallback:', err);
      }
    }, 100);
  };

  // Google Credential Response Handler (Decodes OAuth JWT)
  window.handleGoogleCredentialResponse = function (response) {
    ...
    try {
      ...
      console.log('[*] Google OAuth 2.0 Login Success:', payload);

      window.MobileState.userProfile.name = payload.name || payload.email.split('@')[0];
      window.MobileState.userProfile.email = payload.email;
      if (payload.picture) {
        window.MobileState.userProfile.avatar = payload.picture;
        const avatarEl = document.getElementById('mobileUserAvatar');
        if (avatarEl) avatarEl.src = payload.picture;
      }
      window.MobileState.userProfile.isGoogleConnected = true;
      ...
      if (greetEl) greetEl.textContent = `Good afternoon, ${window.MobileState.userProfile.name.split(' ')[0]}`;

      const labelEl = document.getElementById('googleAccountLabel');
      if (labelEl) labelEl.textContent = `${window.MobileState.userProfile.name} (${payload.email})`;

      document.getElementById('googleAuthModal')?.remove();
      alert(`✅ Successfully signed in with Google as ${payload.name} (${payload.email})!`);
    } catch (e) {
